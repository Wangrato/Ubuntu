  ...
  { if(duty[i]<0)
    ...
    if(duty[i]>600)
      duty[i]=600;
        ...
        cv[0] = (duty[0] * (period[ftm2] - 0 + 1)) / FTM2_PRECISON;
        cv[1] = (duty[1] * (period[ftm2] - 0 + 1)) / FTM2_PRECISON;
        cv[2] = (duty[2] * (period[ftm2] - 0 + 1)) / FTM2_PRECISON;
        cv[3] = (duty[3] * (period[ftm2] - 0 + 1)) / FTM2_PRECISON;
  
    
        FTMX[ftm2]->CONTROLS[ftm_ch2].CnV = cv[0];      //设置占空比
        FTMX[ftm2]->CONTROLS[ftm_ch3].CnV = cv[1];      //设置占空比
        FTMX[ftm2]->CONTROLS[ftm_ch4].CnV = cv[2];      //设置占空比
        FTMX[ftm2]->CONTROLS[ftm_ch5].CnV = cv[3];      //设置占空比 
}
